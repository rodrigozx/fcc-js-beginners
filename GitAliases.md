...
git config --global alias.ghcreateall '!f() { dir=${1:-.}; mkdir -p \"$dir\" && cd \"$dir\" && git init && git config --global --add safe.directory \"$(pwd)\" && git add . && git commit -m \"Initial commit\" && git ghcreate;echo Run: cd \"$dir\"; }; f'
# Initialize and create a new private repository on GitHub
git config --global alias.ghcreateallp '!f() { dir=${1:-.}; mkdir -p \"$dir\" && cd \"$dir\" && git init && git config --global --add safe.directory \"$(pwd)\" && git add . && git commit -m \"Initial commit\" && git ghcreatep;echo Run: cd \"$dir\"; }; f'
